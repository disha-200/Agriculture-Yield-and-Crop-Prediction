...
data['Area']=data['Area'].astype('int') #convert data into integer
data['Production']=data['Production'].astype('int') #convert data into integer

print(data.columns) #list of columns
print(data['Crop'].describe()) #description of perticular column

print(data['Crop'].value_counts()) #no. of counts for each crop
...
print("each state mapped for unique number ",dictionary_states)
...
print("each distrcit mapped for unique number ",dictionary_districts)
...
print("season  names ",season_names)
dictionary_season={'Kharif':1, 'Whole Year':2, 'Autumn':3, 'Rabi':4,
       'Summer':5, 'Winter':6}

print("each season mapped for unique number ",dictionary_season)
data['Season']=data['Season'].map(dictionary_season)
...
joblib.dump(model1, 'final_pickle_model_yield.pkl')

# Load the model from the file
final_model = joblib.load('final_pickle_model_yield.pkl')
